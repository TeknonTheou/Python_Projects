...
def first_run(self):
    data = ('John','Doe','John Doe','111-111-1111','jdoe@email.com', 'Science')
    conn = sqlite3.connect('db_studenttracking.db')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count < 1:
            cur.execute("""INSERT INTO tbl_students (col_fname,col_lname,col_fullname,col_phone,col_email, col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Science'))
            conn.commit()
    conn.close()
...
def addToList(self):
    ...
    if not "@" or not "." in var_email: # will use this soon
        messagebox.showerror("Email Error","'{}' is an incorrect email format!".format(var_email))
...
def onUpdate(self):
    try:
        ...
        return
    # The user will only be alowed to update changes for phone, emails and course.
